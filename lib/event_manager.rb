...
def clean_phonenumber(homephone)
  clean_num = homephone.gsub(/-|\.|\(|\)|\s+/, '') # Remove off spacing
  ...
  # Converts phone num into xxx-xxx-xxxx format
...
def registration_analytics(hash)
  ...
  puts "#{peak_array[0][1]} people signed up at #{hour_convert(peak_array[0][0])}."
  puts "#{peak_array[1][1]} people signed up at #{hour_convert(peak_array[1][0])}."
  puts "#{peak_array[2][1]} people signed up at #{hour_convert(peak_array[2][0])}."
end
...
registration_hours = Hash.new(0) # Hash used for registration hours analytics
...
contents.each do |row|
  ...
  # Upades registration hours hash
  date = row[:regdate]
  format = "%m/%d/%y %H:%M"
